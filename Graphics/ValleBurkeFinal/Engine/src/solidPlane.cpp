...
SolidPlane::SolidPlane(const unsigned pid, unsigned longs, unsigned lats) :
    Plane(pid),
    longitudes(longs),
    latitudes(lats)
{
    ...
    // Initialize plane
    fillPlaneVertexArray(vertices, latitudes, longitudes);
    fillPlaneIndices(indices, latitudes, longitudes);
    fillPlaneCounts(counts, latitudes, longitudes);
    fillPlaneOffsets(offsets, latitudes, longitudes);
...
void SolidPlane::draw(mat4 modelViewMat)
{
